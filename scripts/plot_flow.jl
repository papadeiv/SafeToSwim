...
filename = "flow/Flow_Mean.csv"
df = CSV.read("../data/"*filename, DataFrame)

# Loop over the number of sites
printstyled("Plot and export the flow timeseries at every site\n"; bold=true, underline=true, color=:light_blue)
@showprogress for n in 1:(ncol(df)-1)
        # Import data
        data = flow_parser(filename, n+1)
        ...
        savefig("flow_mean/$(data.site).png", fig)
end
